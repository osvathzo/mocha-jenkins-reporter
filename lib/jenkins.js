...
function Jenkins(runner, options) {
  ...
  function genSuiteReport() {
    ...
    if (currentSuite.tests.length === 0 && currentSuite.failures > 0) {
      ...
      currentSuite.tests.forEach(function(test) {
        ...
        writeString(' name="'+htmlEscape(currentSuite.suite.fullTitle() + ': ' + test.title)+'"');
  ...
  function GetCommaSeparatedSuiteName() {
    if (this.parent) {
  ...
  function startSuite(suite) {
    ...
    currentSuite.suite.fullTitle = GetCommaSeparatedSuiteName;
    console.log();
  ...
  runner.on('fail', function(test, err) {
    ...
    var fmt = indent()
      + color('fail', '  %d) %s');
    console.log(fmt, failureIndex, test.title);
  });
}
